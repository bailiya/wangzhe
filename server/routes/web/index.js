...
//导入新闻数据
	router.get('/news/init', async (req, res) => {
		...
		const newsTitles = ["稷下学院聚餐，星辰小伙用成员付账？", "王者荣耀X稻香村：中秋来王者峡谷赏月品稻香", "联手“乞巧”非遗，《王者荣耀》缔造数字“情人节”", "缘起峡谷，情定七夕 | 我所爱的，都在王者峡谷里", "听说，小妲己偷偷从峡谷溜去云南了？", "8月26日七夕“为爱向前冲”活动维护完成公告", "8月26日外挂专项打击公告", "8月26日“演员”惩罚名单", "8月26日净化游戏环境声明及处罚公告", "8月26日体验服停机更新公告", "荣耀中国节·银河配对，参与必得乞巧牵缘回城特效（新）", "峡谷迎七夕，甜蜜好礼来袭", "恭喜TS夺得世冠总冠军，回馈福利来袭，典韦-蓝屏警告星元上新", "【看世冠对决 赢豪华大礼】活动公告", "应援世冠得好礼，墨子两款皮肤重塑完成即将登场", "王者荣耀电竞推出六大措施打造全民参与体验闭环", "2020年王者荣耀世界冠军杯总决赛门票8月10日正式开售", "8月7日【比赛服】版本更新公告", "7月29日【比赛服】版本更新公告", "7月13日【比赛服】版本更新公告"]
		const newsList = newsTitles.map(title => {
			const randowCats = cats.slice(0).sort((a, b) => Math.random() - 0.5)
			return {
				categories: randowCats.slice(0, 2),
				title: title
			}
		})
...
//展示新闻
	router.get('/news/list', async (req, res) => {
		...
		const cats = await Category.aggregate([
			{$match: {parent: parent._id}},//查找父元素id为“新闻分类”的元素
			{
				$lookup: {
					from:'articles',//关联Article表
					localField: '_id',//本身的键
...
//展示英雄
	router.get('/heroes/list', async (req, res) => {
		...
		//聚合查询
		const cats = await Category.aggregate([
			{$match: {parent: parent._id}},//查找父元素id为“英雄分类”的元素
			{
				$lookup: {
					from:'heroes',//关联Article表
					localField: '_id',//本身的键
					foreignField: 'categories',//外键
					as: 'heroList',//起名字
				},
...
//查找英雄所属分类
	router.get('/herocat/:id', async (req, res) => {
		const data = await Hero.findById(req.params.id)
		const cats = await Hero.find().where({
			name: data.name
		}).populate('categories')
		const category = cats.map(v => v.categories[0])
		const categories = category.map(v => v.name)
		res.send(categories)
	})
